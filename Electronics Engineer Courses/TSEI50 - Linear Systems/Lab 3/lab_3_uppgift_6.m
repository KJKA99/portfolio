...
title('Spectra cos(0.249*pi*n1)+0.01*(abs(cos(0.249*pi*n1)).^2) quantized (10 bits), N = 8192, [dB scale]')
xlabel('N points');
ylabel('dB');

subplot(2,1,2);
plot(L4); %16 bits 0.249
title('Spectra cos(0.249*pi*n1)+0.01*(abs(cos(0.249*pi*n1)).^2) quantized (16 bits), N = 8192, [dB scale]')
xlabel('N points');
ylabel('dB');
figure();
...
subplot(2,1,1);%10 bits 0.249
plot(L7);
title('Spectra cos(0.249*pi*n1)+0.01*(abs(cos(0.249*pi*n1)).^2) quantized (10 bits), N = 8192, [Blackmanharris]')
xlabel('N points');
ylabel('dB');

subplot(2,1,2);%16 bits 0.249
plot(L8);
title('Spectra cos(0.249*pi*n1)+0.01*(abs(cos(0.249*pi*n1)).^2) quantized (16 bits), N = 8192, [Blackmanharris]')
xlabel('N points');
ylabel('dB');
